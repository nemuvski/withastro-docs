...
Astro v5.0では、コンテンツコレクションを定義してクエリするためのContent Layer APIが導入されました。この高性能でスケーラブルなAPIは、ローカルコレクション用の組み込みコンテンツローダーを提供します。リモートコンテンツの場合、サードパーティやコミュニティによって構築されたローダーを使用するか、独自のカスタムローダーを作成して、任意のソースからデータを取得できます。

:::note
// TODO: あとでリンク先を変更
Astro v2.0で導入された従来のコンテンツコレクションAPIを引き続き使用できますが、可能であれば[既存のコレクションを更新](/ja/guides/upgrade-to/v5/#legacy-v20-content-collections-api)することをお勧めします。
:::

## コンテンツコレクションとは？

構造的に類似したデータのセットから**コレクション**を定義できます。これはブログ投稿のディレクトリ、製品アイテムのJSONファイル、または同じ形状の複数のアイテムを表す任意のデータである可能性があります。
プロジェクト内またはファイルシステム上にローカルに保存されたコレクションには、Markdown、MDX、Markdoc、YAML、またはJSONファイルのエントリを含めることができます。
...
- **authors/** "author"コレクション
  - 1つのauthors.jsonファイルにすべてのコレクションエントリーを含める
</FileTree>

コレクションができたら、Astroの組み込みコンテンツAPIを使って[コンテンツのクエリ](#コレクションのクエリ)を始められます。

適切なコレクションローダーを使用すると、CMS、データベース、ヘッドレス決済システムなど、任意の外部ソースからリモートデータを取得できます。

## コレクションのためのTypeScript設定

コンテンツコレクションは、Zodのバリデーション、Intellisense、およびエディタでの型チェックを提供するためにTypeScriptに依存しています。Astroの`strict`または`strictest`のTypeScript設定を拡張していない場合は、`tsconfig.json`で次の`compilerOptions`が設定されていることを確認する必要があります：

```json title="tsconfig.json" ins={5} {6}
{
  // "astro/tsconfigs/strict" または "astro/tsconfigs/strictest" に含まれています
  "extends": "astro/tsconfigs/base",
  "compilerOptions": {
    "strictNullChecks": true, // `base` テンプレートを使用する場合は追加してください
    "allowJs": true // 必須であり、すべてのAstroテンプレートに含まれています
  }
}
```
